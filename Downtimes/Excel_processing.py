        ...
        try:
            ...
            for col in time_columns:
                ...
                self.data[col] = self.data[col].apply(
                    lambda x: (
                        lambda dt: dt.strftime("%H:%M") if dt is not pd.NaT else str(x)
                    )(pd.to_datetime(str(x), format="%H:%M", errors='coerce'))
                    if pd.notna(x) and str(x) not in ('<NA>', 'nan') else str(x)
                )
...
#     db = Database_process()
#     processor = Downtime_Excel_Processor(file_path=r"C:\Users\2173452100291\Downloads\Downtime  SC-A.xlsx", sheet_name="May-26", database=db, area_name="SC-A")
#     data, error_frame, working_time = processor.read_filter_excel()
#     print(data)
#     print(error_frame)
#     print(working_time)